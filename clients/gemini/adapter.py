...
_TOOL_SPECS = [
    ...
    ("approve_campaign_kit", "Trigger explicit human approval for a kit.", {"type": "object", "properties": {"kit_id": {"type": "string"}}, "required": ["kit_id"]}),
    ...
    ("create_campaign", "Run the governed campaign pipeline and stop at human approval.", {"type": "object", "properties": {"brief_data": {"type": "object"}}, "required": ["brief_data"]}),
    ("get_campaign_status", "Return persisted campaign status.", {"type": "object", "properties": {"campaign_id": {"type": "string"}}, "required": ["campaign_id"]}),
    ("resume_campaign", "Resume an interrupted campaign from persisted state.", {"type": "object", "properties": {"campaign_id": {"type": "string"}}, "required": ["campaign_id"]}),
    ("approve_campaign", "Human-approve an orchestrated campaign.", {"type": "object", "properties": {"campaign_id": {"type": "string"}}, "required": ["campaign_id"]}),
    ("export_campaign", "Export an APPROVED orchestrated campaign.", {"type": "object", "properties": {"campaign_id": {"type": "string"}, "export_format": {"type": "string", "enum": ["json", "docx", "xlsx"]}}, "required": ["campaign_id"]}),
]
...
assert {t["name"] for t in TOOL_DEFINITIONS} == set(PUBLIC_TOOL_NAMES)
